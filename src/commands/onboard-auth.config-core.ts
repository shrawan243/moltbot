...
/**
 * Apply Nebius provider configuration without changing the default model.
 * Registers Nebius models and sets up the provider, but preserves existing model selection.
 */
export function applyNebiusProviderConfig(cfg: MoltbotConfig): MoltbotConfig {
  const models = { ...cfg.agents?.defaults?.models };
  models[NEBIUS_DEFAULT_MODEL_REF] = {
    ...models[NEBIUS_DEFAULT_MODEL_REF],
    alias: models[NEBIUS_DEFAULT_MODEL_REF]?.alias ?? "Qwen3 Coder",
  };
...
/**
 * Apply Nebius provider configuration AND set Nebius as the default model.
 * Use this when Nebius is the primary provider choice during onboarding.
 */
